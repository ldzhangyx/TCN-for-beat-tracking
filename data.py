...
class BallroomDataset(Dataset):
    ...
    def __init__(self):
        self.data_list = [item.split('.')[0] for item in config['spec_folder']]
        self.hop_size = int(np.floor(config['hop_size'] * config['sample_rate']))

    def __len__(self):
        return len(self.data_list)

    def __getitem__(self, index):
        data_path = '/'.join([config['spec_folder', self.data_list[index], '.npy']])
        label_path = '/'.join([config['label_folder', self.data_list[index], '.beat']])
